...
def insert_before_first(text: str, anchors: list[str], block: str, label: str) -> str:
    for anchor in anchors:
        ...
        if idx != -1:
            print(f"Inserted: {label} before {anchor[:60]!r}")
...
def main() -> None:
    ...
    if "top-identify-equipment-block-v1" in source:
        print("Top Identify Equipment block already applied.")
        return

    source = "/* top-identify-equipment-block-v1 */\n" + source
...
"""
    ...
    source = insert_before_first(
        ...
        block,
        "Top Identify Equipment block",
    )

    backup = PAGE.with_name(PAGE.name + ".top-identify-equipment-block-v1.bak")
