...
def main() -> None:
    ...
    if state_line not in s:
        ...
        idx = s.find(anchor)
        if idx == -1:
            raise RuntimeError("Could not find state anchor for showFailureDashboard.")
    ...
    replacement = """{/* failure-intelligence-dashboard-v1 */}
...
"""
    ...
    backup = PAGE.with_name(PAGE.name + ".fix-failure-dashboard-toggle.bak")
